...
- 🔴 **쓰기(`post-page`·`patch-block-children`·`patch-page`) 뒤에는 응답의 `plain_text`를 되읽어 대조하는 것이 
  절차의 마지막 단계다 — 선택이 아니다.** 아래 "이스케이프 금지"는 **규범으로 두 세션 연속 안 먹혔다**
  (2026-08-25에 6글자, 2026-08-26에 `BGM 3곡`→**`3곱`**). 규범이 아니라 **단계로** 박는 이유다.
  응답은 **성공으로 돌아오고** 깨진 글자는 그 안에만 있다 — 안 읽으면 사용자 카드에 오타가 그대로 남는다.
  ⚠️ 고칠 때 `API-update-a-block`의 `type` 인자는 **본문을 못 받는다**(400). 잘못된 블록을 `archived: true`로
  지우고 `patch-block-children`의 `after`로 같은 자리에 다시 붙일 것.
- 🔴 **한글은 그대로 쓸 것. 유니코드 이스케이프(`핫…`)를 손으로 만들지 말 것.**
  트리거 자명 — *노션 툴에 한글이 든 문자열을 넘기기 직전.* JSON에 한글을 직접 넣어도 아무 문제가 없다.
  (2026-08-07: 이스케이프로 보낸 한글이 **4번 중 4번** 다른 글자가 됐다 — `핫뚜1`→`핫뙐1`→`핫뜬1`, `만렙`→`만렉`,
  `곡선`→`공선`, `몸이 떠도`→`메가 띄어도`. 사용자가 보는 카드에 오타를 심었고 고치는 데 왕복 3번을 더 썼다.)
  🔴 **본문이 길어도 이스케이프로 도망가지 말 것 — 오히려 두 배가 된다.** 위 규칙은 *왜 깨지나*만 말해서
  **"인자가 잘렸으니 줄여야겠다"는 순간**엔 발동을 안 한다. 이스케이프는 한 글자를 6바이트로 부풀려
  **잘림을 더 빨리 부르고 오타까지 얹는다.** 크기가 문제면 답은 **나눠 보내는 것**이다
  (`replace_content`로 앞부분 → `insert_content`(position end)로 뒤를 잇는다).
  (2026-09-02: 표 72행을 이스케이프로 보내 9.8KB에서 잘렸고, 한글 그대로 다시 보내니 통과했다.
  그 전에 이스케이프로 올린 66행에서는 `흩어지던`→`흔어지던`, `돈다`→`돌다` 두 글자가 틀어져 있었다.)
- ⚠️ **`API-update-page-markdown`은 조용히 매칭에 실패한다.** `update_content`의 `old_str`이 한 건 안 맞아도
  응답은 성공으로 온다 — **반환된 마크다운에서 그 줄이 실제로 바뀌었는지 눈으로 확인할 것.**
