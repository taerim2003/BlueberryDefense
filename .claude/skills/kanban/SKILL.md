...
5. **보드는 사용자와 공동 관리한다.**
   - 사용자는 **세션과 무관하게 아무 때나** 카드를 올린다 → 세션 시작에 반드시 다시 조회(CLAUDE.md §7-0-1). 세션이 길어지면 중간에도 한 번 더 볼 것.
...
- **세션 시작에 조회(CLAUDE.md §7-0-1), 세션 종료에 정리(`wrap` 스킬).** 끝난 카드는 `완료`로 옮긴다 — 안 옮기면 다음 세션이 끝난 일을 또 한다.
