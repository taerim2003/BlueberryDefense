...
- **새로 만든 .cs 파일을 컴파일러가 전혀 인식하지 못하는 경우가 있었다** (AssetDatabase엔 잡히는데 `CompilationPipeline`의 sourceFiles엔 안 잡힘). 당시 우회책으로 새 클래스를 기존 .cs 파일 하단에 병합했었는데 — **이 우회책 자체가 빌드 전용 크래시를 낳는 진짜 버그였다.** 아래 항목 참고. 다음에 "새 파일을 컴파일러가 안 잡는" 증상이 재발하면, 병합 우회책 대신 먼저 `assets-refresh`를 한 번 더 시도하고 그래도 안 되면 사용자에게 Claude Code/Unity 재시작을 요청할 것 — 병합은 최후의 수단으로만.
- **⚠️ 치명적: 한 .cs 파일에 여러 MonoBehaviour 클래스를 넣는 "병합" 패턴은 에디터에서만 동작하고 빌드에서는 크래시한다.** 파일명과 다른 이름의 보조 클래스는 `m_Script: {fileID: 0}` + `m_EditorClassIdentifier: Assembly-CSharp::ClassName`로 직렬화되는데, **에디터 Inspector는 이 클래스이름표를 보고 관대하게 재연결해주지만 빌드된 플레이어는 이 fallback이 없어 컴포넌트 자체가 안 붙는다.** 결과: `GetComponent<T>()`가 조용히 null → 그 즉시 NullReferenceException, 그런데 에디터 콘솔·플레이테스트에서는 멀쩡히 잘 작동해서 전혀 못 알아챔 (실제로 `DamageNumber`/`HitParticle`을 `Enemy.cs`에 병합해뒀다가 빌드에서만 크래시, 몇 세션 동안 몰랐음). **새 클래스는 반드시 파일명과 동일한 이름의 별도 .cs 파일로 만들 것.** 이미 병합된 게 있으면 분리 후 `MonoScript.FromMonoBehaviour`나 `AssetDatabase.LoadAssetAtPath<MonoScript>(경로)`로 얻은 정식 스크립트 참조로 프리팹의 `m_Script`를 재연결해야 함 (fileID는 파일명과 일치하는 주 클래스일 때 항상 `11500000`).
- **에디터에서는 재현 안 되고 빌드에서만 나는 버그는 추측하지 말고 `Player.log`부터 볼 것.** 위치: `%USERPROFILE%\AppData\LocalLow\<CompanyName>\<ProductName>\Player.log`. `NullReferenceException`/`MissingReferenceException`/`missing!` 같은 키워드로 검색하면 스택 트레이스가 그대로 나온다 — 이번 세션의 원인도 로그 한 번에 바로 확정됨. Development Build + Script Debugging 체크하면 게임 화면에서 실시간 콘솔도 볼 수 있음.
- **HashSet/List 같은 컬렉션을 `foreach`로 순회하면서 그 안에서 컬렉션의 항목을 죽이거나 파괴하는 메서드(`Destroy(gameObject)` 등)를 호출하면 `InvalidOperationException: Collection was modified`가 날 수 있다.** `Destroy()`가 Collider2D를 비활성화하면서 `OnTriggerExit2D`를 동기적으로 발생시킬 수 있고, 그 콜백이 순회 중인 컬렉션(`overlappingEnemies.Remove()` 등)을 건드리면 즉시 터짐. 트리거 추적용 HashSet을 순회하며 데미지를 주는 틱 데미지 스킬(회오리·오브류)에서 특히 잘 남 — `foreach (var x in new List<T>(originalSet))`처럼 스냅샷 복사본을 순회할 것.
