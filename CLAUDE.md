...
- **목표를 비율(%)로 받으면, 구현 전에 그 %가 가리킬 수 있는 축을 전부 재서 나란히 놓는다**(개수·비용·거리·시간 등).
  ...
  **동작은 맞는데 체감이 정반대**로 나온다. 재 보면 어느 축에서 그 %가 "자연스러운 자리"에 떨어지는지가 보인다.
  (2026-09-14: 스킬트리 "10%·50% 해금하면 진화 등장"을 **노드 개수 게이트**로 구현했는데 뜻은 **루트에서의 거리**였다 —
  거리 분포를 한 번 쟀으면 "거리 2 = 13%"가 바로 보였다.)
...
무인 반복은 `/loop /balance-loop [최대이터레이션] [마감HH:MM]` — 봇 세션이 도는 동안 다른 세션은 Unity 컴파일·플레이모드를 쓰지 않는다.
...
- 🔴 **거꾸로도 틀린다 — enum·에셋에 남아 있다고 게임에 있는 게 아니다. *진입점 배열*에서 센다.**
  ...
  (2026-09-08: 패시브를 enum으로 세어 7종이라 보고했는데 `LevelUpUI`의 후보 배열은 **6종**이었다.
  리프레쉬는 2026-08-06에 폐지됐고 그 배열 바로 위에 주석까지 있었다.)
- 🔴 **설계를 위임받아도 "무엇이 불가능한가"는 내가 조사한다.**
  ...
  시작 스킬 3종은 다른 캐릭터가 못 얻는다 — `LevelUpUI`의 후보 배열 한 줄 grep이면 나왔다.)
- 🔴 **"아무도 안 부른다"를 말하기 전에 *에디터 전용 호출부*를 따로 센다.** 위 두 규칙은 *게임에 그게 있나*를 말해서
  **코드를 지워도 되나**엔 발동을 안 한다 — 씬·프리팹 GUID와 런타임 참조만 재고 `Assets/Editor/`를 빠뜨리기 쉽다.
  ...
  (2026-09-08: 조사 에이전트 셋이 입을 모아 "폐지된 리프레쉬 분기는 죽었다"고 했지만 `CheatWindow`가
  enum 전체를 순회해 획득 버튼을 만들고 있었다. 지우기 직전에 열어 본 덕에 테스트 경로를 안 부쉈다.)
