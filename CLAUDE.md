...
- **파일명 접미사의 뜻을 파일 종류마다 확인할 것.** 세션35에 `Icon_*R1/R2`는 **루트**(R1=루트0)였는데 `Effect_Arrow*R1/R2`는 **티어**(R1=1차 진화)였다 —
  같은 접미사가 다른 뜻이었고, 아이콘 쪽 해석을 그대로 밀었으면 이펙트가 어긋났다. **그림을 열어보는 것으로는 안 갈린다. 한 번 물을 것.**
