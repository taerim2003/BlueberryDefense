...
### 📋 할 일의 원본 = 노션 칸반보드 "POC 할일"
> https://app.notion.com/p/3b06394cd983803daf4ac26f924121b7 (DB id `3b06394c-d983-803d-af4a-c26f924121b7`)
> 상태: `시작 전` / `진행 중` / `완료`. **여기가 원본이고 아래 "다음 할 일"은 사본이다.**
> 어긋나면 칸반이 이긴다. 세션 종료 시 끝난 카드는 `완료`로 옮길 것.
...
> ⚠️ 카드 4건 중 3건(새 캐릭터·농장맵 확장·맵 해금)은 **콘텐츠 변경**이라 "POC 이후 게임 구성 불변" 원칙과 형식상 충돌한다.
> 사용자가 POC 종료 후 직접 올린 카드이므로 **결정된 것으로 본다**. 다시 묻지 말 것.
