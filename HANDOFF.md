...
## 현재 상태 (2026-08-26, 세션 49)

**빌드**: 컴파일 통과. 세션48에서 통리뷰 9건, 세션49에서 **8/25 회의록 액션 아이템 미체크 12건**(2건은 이미 돼 있었고 10건 수정).
항목별 상세는 칸반 **「08/26 통리뷰 수정 9건」(완료)** · **「08/25 — 밸런스 수치·스크린샷·상점 심사 제출」(진행 중)** 카드 본문.
여기엔 코드로 남는 함정만 적는다. **회의록 체크박스는 API로 못 건드린다 — 태리미가 손으로 체크해야 한다.**

- 🔴 **판 그림의 *투명 여백*과 *둥근 실루엣*은 §5-1의 "칸 비율 vs 그림 비율" 검사에 안 걸린다.**
  비율이 맞아도 그림이 칸 전체를 안 채우므로 **rect 기준으로 놓은 자식이 판 밖 허공에** 간다
  (일시정지 창: 제목이 판 위로, 버튼·ESC 힌트가 판 아래로 나가 있었다).
  → 실루엣을 **PNG 알파로 재서** 그 안에 배치한다. `베개같이생긴네모_색칠`은 373×195 중 채워진 면이 x34~344 · y22~149뿐이고,
  둥글어서 **위아래 끝일수록 쓸 수 있는 가로폭이 좁다**(측정값은 `PauseMenu.BuildUI` 위 주석에 표로 있다).
- 🔴 **`ObjectPool`은 `localScale`을 되돌려 주지 않는다.** 풀링 VFX 크기를 코드로 바꾸면 재사용본이 옛 크기로 나온다 —
  **스폰마다 직접 넣을 것**(`LightningRodRoutine`의 `boltScale`).
- 🔴 **딤이 안 먹은 것처럼 보여도 값부터 의심하지 말 것 — URP는 리니어 공간에서 합성한다.**
  알파 0.8짜리 `#08050D` 딤이 화면에선 체감 **약 50%**로만 어두워진다(실측: 배경 231 → 112). 정상이다.
- 🔴 **`SmallOrb.Source` 기본값이 `Orb`다.** 이 프리팹을 다른 스킬이 쓰면 반드시 갈아야 데미지 미터가 맞는다
  (산탄 피해가 통째로 오브로 잡히던 원인).
- 🔴 **진화 티어(`PathTier[n]`)로 분기할 땐 스킬 종류(`Id`)를 같이 볼 것.** 티어만 보면 **다른 스킬이 걸린다**
  (호밍 "소형 미사일 다발" 레벨업 카드에 "화살비 +N차례"가 떴다).
- ⚠️ **이펙트 프리팹 `localScale`은 전부 `1.5`여야 한다**(§7 픽셀 컨벤션). `Effect_LightningRod`만 `1`이라 그것만 작았다.
  새 이펙트를 배선할 때 이 값부터 볼 것.
