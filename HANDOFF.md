...
> (세션27은 게임 코드를 안 고친 **파이프라인 정비** 세션이었다 — `CLAUDE.md` 244→159줄, 절차를 스킬 3종으로 분리(`kanban`·`unity-mcp`·`wrap`), 서브에이전트는 안 쓰기로 확정.
> 키트 스킬은 `prototyping-kit/skills/`가 원본이고 `~/.claude/skills/`에 정션으로 연결돼 있다.)
...
## 다음 할 일 — 칸반보드 (2026-08-04 조회: `시작 전` 4 / `진행 중` 4 / `완료` 1)
