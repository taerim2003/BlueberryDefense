...
**① 보물상자로 스킬 진화** — 진화 획득 경로를 보물상자 하나로 통합 (볼x핏 방식)
- 지금: 보스 판 클리어 → 보라 진화 아이템 드롭 → `EvolutionItemPickup` → `LevelUpUI.ShowEvolutionReward()`. **보물상자(`ShowTreasureReward()`)와 완전히 별개 경로.**
- 목표: 보물상자 획득 시 진화 가능한 스킬 수로 분기
  - 0개 → 현행대로 랜덤 스킬 1~5회 레벨업
  - 1개 → "레벨업 vs 진화" 선택
  - 2개 이상 → 진화 선택 → 어느 스킬 진화할지 선택 → 기존 진화창(`EvolutionTreeUI`)
- 손댈 곳: `LevelUpUI.cs`의 `ShowTreasureReward`/`ShowTreasure`(374~) + `ShowEvolution`/`BuildEvolutionOptions`(554~). 진화 아이템 드롭 자체를 없앨지 결정 필요.
...
**③ 농장맵 넓히기** — 농장이 해변보다 재미없다는 판단
- 농장 `fieldScale` 1 → **1.2~1.3**. 확정 후 셋째 맵 배율 재계산(원래 1.5×1.5=2.25배였는데 그 아래로 낮추는 게 목적).
- 배경 픽셀 규격 산출이 선행: PPU **18** 기준 1배=320×180이므로 1.25배면 **400×225**.
- 크기만이 아니라 **구성도** 바꾼다 — 중간 스폰·UFO 투하 등 **게릴라성 등장 비율을 높인다**(`ambushCount`는 해안가에서만 금지, 농장은 대상).

**④ 새 캐릭터 관련 피쳐** — 캐릭터는 아직 `Char_Strawberry` **1종뿐**
...
- **`Map_Wide20`(광활한 밭)** — 배경이 없으면 맵 자체가 미완성. **카드 ③이 이 맵의 배율을 바꾸므로 ③ 확정 전에 배경을 그리면 헛일이다.** 순서: ③ 농장 배율 확정 → 셋째 맵 배율 산출 → PPU 18로 배경 임포트 → `background` 배정 → `cameraYLift` → 전용 스테이지 테이블.
