...
public class PlayerPassives : MonoBehaviour
{
    ...
    // 문장은 번역 표(`Assets/Localization/Tables/Game`)가 소유한다. 키 = evo.passive.desc.{패시브}.{루트}.{티어}.
    // 정의가 없는 조합은 빈 문자열 — 옛 `_ => ""` 분기와 같다(표에 키가 없으면 Has가 false다).
    public static string DescribePathEffect(PassiveSkillId id, int path, int tier) =>
        Loc.TOr($"evo.passive.desc.{id}.{path}.{tier}", "");

    // 진화 카드에 붙는 짧은 제목. 기능명이 아니라 별명이다.
    public static string GetPathTierTitle(PassiveSkillId id, int path, int tier) =>
        Loc.TOr($"evo.passive.title.{id}.{path}.{tier}", "");
}
