...
public class StageTable : ScriptableObject
{
    ...
    // ── 저작 범위 밖(승천으로 길어진 판) 자동 생성 ──
    // 승천이 오르면 판이 20·25·30 스테이지로 길어지는데 손으로 저작한 엔트리는 15개뿐이다.
    // 그 뒤는 마지막 저작 엔트리에서 출발해 스테이지당 아래 비율로 계속 키운다.
    ...
    // 🔴 현재 성장률은 **의도적으로 가파르다**(사용자 결정, 세션17). 승천3 최종이 기준(승천1 15스테이지)의 약 85배.
    //    되돌리지 말 것 — 버그가 아니라 "일단 세게 잡고 플레이로 깎는다"는 선택이다.
    //    ⚠️ 참고: 만렙 10 + 진화 2회로 플레이어 파워엔 상한이 있어서, 판이 길어져도 플레이어는 그 이상 안 세진다
    //    (늘어나는 건 스킬트리뿐). 못 깨겠으면 제일 먼저 내릴 손잡이가 여기다. 완만안은 1.08(=승천3 6.6배).
