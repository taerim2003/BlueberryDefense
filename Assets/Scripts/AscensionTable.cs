...
// 요청 범위대로 적 체력·이동속도·데미지 3가지만 조절한다. 인스펙터/Balance Dashboard에서 수치 편집.
// RunConfig.AscensionLevel로 이번 판 등급을 선택하고, EnemySpawner가 스폰 시 이 배율을 곱한다.
[System.Serializable]
...
public class AscensionTable : ScriptableObject
{
    ...
    {
        new AscensionTier { hpMult = 1f,    speedMult = 1f,   damageMult = 1f },    // 승천 1 (현재 난이도)
        new AscensionTier { hpMult = 1.35f, speedMult = 1.1f, damageMult = 1.25f }, // 승천 2
        new AscensionTier { hpMult = 1.8f,  speedMult = 1.2f, damageMult = 1.5f },  // 승천 3
    };
