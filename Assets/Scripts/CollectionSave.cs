...
// 판을 넘어 남아야 하므로 SaveStore에 CSV 한 줄로 담는다(정수·승천 해금과 같은 저장소).
//
// 캐시를 안 두는 게 의도다. 설정의 "세이브 초기화"가 SaveStore.DeleteAll()로 통째로 비우는데,
...
public static class CollectionSave
{
    ...
    private static void Add(string token)
    {
        ...
        SaveStore.SetString(Key, csv.Trim(',') + (csv.Length > 2 ? "," : "") + token);
        SaveStore.Save();
    }

    // 앞뒤에 구분자를 붙인 형태로 돌려준다 — 첫 항목·마지막 항목도 같은 규칙으로 찾히게.
    private static string Csv() => "," + SaveStore.GetString(Key, "") + ",";
}
